...
Initially the goal was to use a non-linear optimization framework to find the optimal hedging cost for a given target VAR, however I was unable to find a framework that could consistently converge to a proper solution. As such, I opted instead to use a brute force optimization strategy where by 5000 simulations would be run with randomly assigned hedging ratios, and the top performing portolfio would be chosen as the optimal solution.

### Objective Function

$\begin{aligned}
& {\text{minimize}}
& & \sum_{n=0}^{n =500} hedgingCost  \\
& \text{subject to}
& & VAR = Target\ VAR
\end{aligned}$
...
hedging_function <- function(aud_hedge_ratio, cad_hedge_ratio, cny_hedge_ratio, eur_hedge_ratio,
  ...
  opt_df_hedged <- opt_df |> 
    ...
    mutate(opt_non_hedged_cash_aud = (rev_current_daily_aud*(1 - opt_hedge_ratio_aud)*exchange_rate_current_aud)/simulated_rate_aud,
           opt_non_hedged_cash_cad = (rev_current_daily_cad*(1 - opt_hedge_ratio_cad)*exchange_rate_current_cad)/simulated_rate_cad,
           opt_non_hedged_cash_cny = (rev_current_daily_cny*(1 - opt_hedge_ratio_cny)*exchange_rate_current_cny)/simulated_rate_cny,
           opt_non_hedged_cash_eur = (rev_current_daily_eur*(1 - opt_hedge_ratio_eur)*exchange_rate_current_eur)/simulated_rate_eur,
           opt_non_hedged_cash_gbp = (rev_current_daily_gbp*(1 - opt_hedge_ratio_gbp)*exchange_rate_current_gbp)/simulated_rate_gbp,
           opt_non_hedged_cash_jpy = (rev_current_daily_jpy*(1 - opt_hedge_ratio_jpy)*exchange_rate_current_jpy)/simulated_rate_jpy) |> 
...
opt_hedge_cost_pct <- .005

# Set Target Var
target_var_ratio <- c(.04, .03, .02, .015)

# Set Min Hedging
min_hedge_pcts <- c(0, .1, .25, .5)
...
optimized_sims_clean <- optimized_sims |> 
  group_by(target_var) |> 
  arrange(desc(opt_total_hedge_cost_mean)) |> 
  slice(1L) |> 
  ungroup()
...
The key takeaways from the optimization are the following:

1.  Based on forecast exchange rates, the unhedged position leads to the greatest mean revenue
2.  VAR targets at 3% and above maintain higher than baseline revenue, while targets below 2% are below baseline revenue.

```{r calculate_optimal_hedging}
#| include: false

# Get Optimized Hedge Ratios
optimized_hedge_ratios <- subset(optimized_sims_clean, target_var == .015) |> select(contains('ratio'))

# Get Zero Hedge Simulation
optimized_sim_hedge <- hedging_function(aud_hedge_ratio = optimized_hedge_ratios$aud_hedge_ratio,
...
optimized_hedging_joint <- optimized_sim_hedge |> 
...
optimized_sims_clean |> 
  ...
  gt::cols_label(target_var = 'Target VAR Pct',
                 ...
                 opt_total_cash_baseline_mean = 'Basline Revenue',
  ...
  gt::tab_spanner(label = 'Optimized Hedge Performance', columns = contains('opt')) |>
  # Format Values
  gt::fmt_percent(columns = contains(c('ratio')), decimals = 2) |> 
  gt::fmt_currency(columns = contains(c('mean','total_var')), decimals = 0) |> 
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(target_var,jpy_hedge_ratio))
  )

```

```{r optimal_hedging_plot}
#| echo: false
...
optimized_hedging_joint |> 
  ...
  geom_vline(xintercept = optimized_hedging_joint |> 
             ...
             linetype = 'dashed', 
             lwd = 1) +
  xlab('Optimized Total Revenuye') +
  ggtitle('Hedged vs. Non-Hedged Portfolio') 
...
hedge_regression |> 
  broom::tidy() |> 
  mutate(p.value = round(p.value, 15)) |> 
  gt::gt() |> 
