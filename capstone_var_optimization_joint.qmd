...
optimized_sims_clean <- optimized_sims_clean |> 
...
optimized_sims_clean |> 
  ...
  select(-c(var_reduction, baseline_var)) |> 
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(jpy_hedge_ratio))
  )
...
optimized_hedging_joint |> 
  ...
  ggtitle('Hedged vs. Non-Hedged Portfolio (VAR Target .04%)') 
...
optimized_hedging_joint |> 
  ...
  geom_vline(xintercept = optimized_hedging_joint |> 
             ...
             lwd = 1) +
  xlab('Optimized Total Revenuye') +
  ggtitle('Hedged vs. Non-Hedged Portfolio (VAR Target .03%)')
...
optimized_hedging_joint |> 
  ...
  geom_vline(xintercept = optimized_hedging_joint |> 
             ...
             lwd = 1) +
  xlab('Optimized Total Revenuye') +
  ggtitle('Hedged vs. Non-Hedged Portfolio (VAR Target 2%)') 

# Target Var .015
optimized_hedging_joint |> 
  filter(VAR_Target %in% c(.015,0)) |> 
  mutate(category = factor(Hedged, levels = c('Yes','No'))) |> 
  ggplot(aes(x = opt_total_revenue, fill = Hedged)) +
  geom_histogram(alpha = .5, bins = 50,  position = 'identity') +
  geom_vline(xintercept = optimized_hedging_joint |> 
  ...
  geom_vline(xintercept = optimized_hedging_joint |> 
             ...
             lwd = 1) +
  xlab('Optimized Total Revenuye') +
  ggtitle('Hedged vs. Non-Hedged Portfolio (VAR Target 1.5%)')
